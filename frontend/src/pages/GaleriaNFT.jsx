...
// Tarjeta individual con efecto de inclinación (tilt) siguiendo el cursor.
const NftCard = ({ nft, idx }) => {
  ...
  return (
    <motion.article
      ...
      className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-zinc-950/95 via-zinc-900/85 to-black/90 transition-colors duration-300 hover:border-violet-400/40 hover:shadow-[0_22px_50px_-22px_rgba(139,92,246,0.55)]"
    >
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-zinc-900">
        ...
        <span className="absolute right-2.5 top-2.5 rounded-full border border-white/15 bg-black/55 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.18em] text-white backdrop-blur-md">
          ERC-721
        </span>
        <span className="absolute left-2.5 top-2.5 rounded-full border border-violet-400/40 bg-violet-500/20 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.18em] text-violet-100 backdrop-blur-md">
          #{nft.id}
        </span>
      </div>
      <div className="space-y-3 border-t border-white/5 p-4">
        ...
        {nft.attributes?.length > 0 && (
          <div className="space-y-1.5">
            {nft.attributes.slice(0, 3).map((attr, i) => (
              <div
                key={`${nft.id}-${i}`}
                className="flex items-center justify-between rounded-lg border border-white/5 bg-white/[0.04] px-2.5 py-1.5 text-[11px]"
              >
        ...
        <a
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-violet-400/30 bg-violet-500/10 py-2 text-[11px] font-semibold text-violet-100 transition-colors hover:bg-violet-500/20"
        >
        ...
        {!isConnected ? (
          ...
          <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-[360px] rounded-2xl border border-white/10 bg-white/[0.03] animate-pulse" />
            ))}
          </div>
        ) : error ? (
